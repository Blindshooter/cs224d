...
def normalizeRows(x):
    ...
    tmp = np.sqrt(np.sum(x*x, axis = 1))
    ### END YOUR CODE
    
    return x/tmp.reshape((x.shape[0], 1))
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    yhat = prob
    yhat[target] = yhat[target]-1 
    gradPred = yhat.dot(outputVectors)

    grad = yhat.reshape((yhat.shape[0],1)).dot(predicted.reshape((predicted.shape[0],1)).T)
